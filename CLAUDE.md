...
Profile updates use `PATCH /api/users/{user_id}` with `UserUpdate` (`name`, `email`, and `password` all optional; validated `min_length`/`max_length` and lowercased email). It returns `{ message, data: { user } }`. Note: this endpoint has **no ownership check and no current-password verification** — a password change only requires a valid bearer token. The frontend surfaces this limitation honestly rather than hiding it.
...
- `GET /api/investigations` — paginated list **scoped to the authenticated user**, ordered `created_at DESC` (newest first). Accepts `limit`/`offset` (via `PaginationParams`) plus optional `search` (matches `company_ticker`, `index_code`, or `question`, case-insensitive) and `investigation_type` (validated against the `InvestigationType` enum; invalid values return 422). Returns the standard envelope `{ message, data: { investigations, pagination } }`.
...
- `components/`:
  - `ui/`: shadcn/ui primitives (`Button`, `Card`, `Badge`, `Input`, `Label`, `Select`, `Avatar`, `DropdownMenu`, `Tabs`, `Table`, `Dialog`, `Pagination`) using `class-variance-authority` and `cn()`. `Select` is a dependency-free, accessible wrapper around the native `<select>` (shared 48px field tokens, accent focus ring). `Pagination` is a presentational, accessible Previous/Next control (`{ total, limit, offset, onOffsetChange, isLoading? }`) using the neutral `secondary` button variant. `Tabs`, `Table`, and `Dialog` are lightweight, dependency-free primitives built on React context (matching the `DropdownMenu` pattern) with ARIA semantics; `Dialog` supports `asChild` on its trigger.
...
- `hooks/`: Reusable hooks including:
  ...
  - `useInvestigations`: Owns the `/investigations` list state — 300ms-debounced search, `investigation_type` filter, 10-item pagination, and a `refetch()` nonce. Every request is `AbortController`-abortable so a superseded response can never win. `loading` is **derived** (current request key vs. last settled key) rather than set synchronously inside the effect, satisfying the `react-hooks/set-state-in-effect` rule. All filtering/paging is server-side; it never fabricates rows and surfaces the real backend error message.
- `lib/`:
  ...
  - `investigationLabels.ts`: Display mappings (labels + semantic badge variants) for investigation type, alignment, confidence, impact, driver type, evidence type, and status, plus no-clear-catalyst detection and evidence aggregation helpers. No business logic — purely presentation.
...
- `models/`: TypeScript interfaces and domain types:
  - `api.ts`: Backend error envelope types (`ApiErrorDetail`, `ApiErrorResponse`). The generic `ApiResponse`/`PaginatedResponse`/`PaginationMeta`/`PaginationParams` types were removed — each domain model defines its own typed envelope that matches the real backend shape.
  - `auth.ts`: User, token, request schemas, and `AuthState` (`user`, `token`, `isAuthenticated`, `isLoading`, `isInitialized`, `error`). Also `UserUpdateRequest` (`name`/`email`/`password`, all optional) and `UserResponse` (`{ data: { user } }`) for the profile PATCH contract.
  ...
  - `investigation.ts`: Investigations, ranked drivers, and tri-state evidence items (`supporting`, `contradictory`, `neutral`). Enums mirror `backend/app/models/enums.py` exactly (investigation type/status, confidence, impact level, driver type, evidence type, alignment) — do not add values the backend cannot emit. Also holds the typed response envelopes, the list contract (`InvestigationListParams`, `InvestigationListResponse` — envelope `{ message, data: { investigations, pagination } }`), and the `CompanyMarketContext` model; `CompanyImpact`/`CompanyImpactResponse` live in `market.ts`.
...
- `pages/`: Route page components:
  ...
  - `investigations/index.tsx`: Investigations list workspace supporting `?ticker=` query param. Renders a searchable, type-filterable, paginated list scoped to the authenticated user: a search `Input`, a `Select` type filter, a result count, and Day.js dates (`formatDate` + `timeAgo`). Distinct loading (skeleton rows), error (`ErrorDisplay` + retry), and contextual empty states (no investigations vs. no matches for the active filters, with a "Clear filters" action). Responsive: a `Table` on `md+` collapsing to a stacked card list on small screens. Rows/buttons open `/investigations/:id`. Hosts the `NewInvestigationDialog` (validated with `analyzeRequestSchema`) that runs `POST /investigations/analyze` and navigates to the created report; a failed analyze shows the real backend error inline and never creates placeholder results.
  ...
  - `profile.tsx`: Authenticated user profile and session settings. Shows identity, account details (definition list, no nested cards), and a single `destructive` sign-out. Adds a **Profile details** form (name/email) and a **Change password** form, both submitted via `PATCH /users/{id}` and surfacing the real backend error inline; the profile update dispatches `updateUser` to refresh Redux. The change-password section honestly notes that the endpoint does not currently verify the existing password. No fabricated saved state.
  - `not-found.tsx`: 404 fallback page.
- `redux/`:
  - `store.ts`: Redux Toolkit store configured with the `auth` slice only. (The unused `investigation` slice was removed — no action was ever dispatched.)
  - `hooks.ts`: Typed `useAppDispatch` and `useAppSelector`.
  - `slices/`: State slices (`authSlice.ts` with `isInitialized` boot flag, `updateUser` reducer, and extraReducers).
  - `thunks/`: Async thunks (`authThunks.ts`: `loginUser`, `registerUser`, `fetchCurrentUser`, `logoutUser`).
- `schema/`: Zod validation schemas for forms (`auth.ts` for login/registration plus `profileUpdateSchema` and `changePasswordSchema` (with a `refine` equality check) for the profile page; `investigation.ts` for queries).
